...
//////////////////////////////////////////////////////
	...
	HEADER
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(o);
			...
			#ifdef MK_NATURAL
				BLOOM_RENDER_TARGET = half4(NaturalRel(BLOOM_RENDER_TARGET.rgb, LUMA_SCALE), 1);
			#else
			...
			*/
			...
			{ 
				...
				#ifdef MK_NATURAL
					LENS_FLARE_RENDER_TARGET += half4(NaturalRel(SampleTex2D(PASS_TEXTURE_2D(_SourceTex, sampler_SourceTex), offset).rgb, LUMA_SCALE).rgb * weight, 0) * LENS_FLARE_GHOST_INTENSITY;
				#else
			...
			#ifdef MK_NATURAL
				LENS_FLARE_RENDER_TARGET += half4(NaturalRel(SampleTex2D(PASS_TEXTURE_2D(_SourceTex, sampler_SourceTex), LENS_FLARE_UV + haloSize).rgb * weight, LUMA_SCALE), 0);
			#else
			...
			#endif
			#ifdef COLORSPACE_GAMMA
				LENS_FLARE_RENDER_TARGET = GammaToLinearSpace4(LENS_FLARE_RENDER_TARGET);
			#endif
		#endif
			...
			#ifdef MK_NATURAL
				GLARE0_RENDER_TARGET = half4(NaturalRel(GLARE0_RENDER_TARGET.rgb, LUMA_SCALE), 1);
			#else
				GLARE0_RENDER_TARGET = half4(LuminanceThreshold(GLARE0_RENDER_TARGET.rgb, GLARE_THRESHOLD, LUMA_SCALE), 1);
			#endif
			#ifdef COLORSPACE_GAMMA
				GLARE0_RENDER_TARGET = GammaToLinearSpace4(GLARE0_RENDER_TARGET);
			#endif
		#endif
