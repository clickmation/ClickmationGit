...
/// </summary>
[Serializable]
[PostProcess(typeof(ShockWaveWSRenderer), PostProcessEvent.BeforeStack, "Custom/ShockWave_PPWS")]
public class ShockWaveWSSettings : PostProcessEffectSettings
{
    /// <summary>
    /// the blur amount
    /// </summary>
...
public class ShockWaveWSRenderer : PostProcessEffectRenderer<ShockWaveWSSettings>
{
    ...
    {
        _globalSWWSTexID = Shader.PropertyToID("_GlobalSWWSTex");
        _SWWSShader = Shader.Find("Hidden/ShockWave_PPWS");
        base.Init();
    }
    ...
    {
        var sheet = context.propertySheets.Get(_SWWSShader);
        ...
        {
            context.command.GetTemporaryRT(_globalSWWSTexID,
                context.camera.pixelWidth >> settings.DownRes,
                context.camera.pixelHeight >> settings.DownRes,
                0, FilterMode.Bilinear, RenderTextureFormat.ARGBFloat);
            context.command.SetRenderTarget(_globalSWWSTexID);
            context.command.ClearRenderTarget(false, true, Color.clear);

        }

        ShockWaveWSManager.Instance.PopulateCommandBuffer(context.command);
