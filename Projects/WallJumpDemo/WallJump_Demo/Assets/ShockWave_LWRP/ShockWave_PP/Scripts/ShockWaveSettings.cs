...
/// </summary>
[Serializable]
[PostProcess(typeof(ShockWaveRenderer), PostProcessEvent.BeforeStack, "Custom/ShockWave_PPM")]
public class ShockWaveSettings: PostProcessEffectSettings
{
...
public class ShockWaveRenderer: PostProcessEffectRenderer<ShockWaveSettings>
{
    ...
    {
        _globalShockWaveTexID = Shader.PropertyToID("_GlobalShockWaveTex");
        _SWShader = Shader.Find("Hidden/ShockWave_PPM");
        base.Init();
    }
    ...
    {
        var sheet = context.propertySheets.Get(_SWShader);
        ...
        {
            context.command.GetTemporaryRT(_globalShockWaveTexID,
            ...
            context.command.SetRenderTarget(_globalShockWaveTexID);
            context.command.ClearRenderTarget(false, true, Color.clear);

        }

        ShockWaveManager.Instance.PopulateCommandBuffer(context.command);
