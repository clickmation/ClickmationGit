...
namespace MK.Glow
{
    ...
    internal sealed class Effect
    {
        ...
        private void CheckFeatureSupport()
        {
            ...
            if(_settings.allowLensFlare && Compatibility.CheckLensFlareFeatureSupport())
                _useLensFlare = true;
            else
                _useLensFlare = false;
            
            //Check if Glare is supported
            if(_settings.allowGlare && Compatibility.CheckGlareFeatureSupport() && _settings.quality != Quality.VeryLow)
                _useGlare = true;
            else
                _useGlare = false;
        ...
        internal void Build(RenderTarget source, RenderTarget destination, Settings settings, CommandBuffer cmd, Camera renderingCamera)
        {
            ...
            if(_settings.workflow == Workflow.Selective)
            {
                ...
                _selectiveRenderTarget.renderTexture = RenderTexture.GetTemporary(_renderingCamera.pixelWidth, renderingCamera.pixelHeight, 16, _renderTextureFormat, RenderTextureReadWrite.Default, 1);//PipelineExtensions.GetTemporary(_selectiveRenderContext, _renderTextureFormat);
