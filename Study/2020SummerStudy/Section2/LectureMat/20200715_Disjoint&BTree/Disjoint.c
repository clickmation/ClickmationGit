...
void Insert(BTTree *Tree, DATATYPE Data, DATATYPE Parent)
{
    AddressNode *newAdressNode = (AddressNode*)malloc(sizeof(AddressNode));
    newAdressNode->Data = Data;
    newAdressNode->Parent = Parent;
    newAdressNode->next = NULL;

    if (Tree->Head->next == NULL) {
        Tree->Head->next = newAdressNode;
        Tree->Tail = newAdressNode;
    } else {
        Tree->Curr = Tree->Tail;
        Tree->Curr->next = newAdressNode;
        Tree->Tail = newAdressNode;
    }
...
DATATYPE Search(BTTree *Tree, DATATYPE *Data)
{
    ...
    if(PosHead(Tree, &Target)) {
        while (Target != *Data)
        {
            if (!PosNext(Tree, &Target)) {
                return NULL;
            }
        }
...
DATATYPE MakeAddressMapWithSubTree(BTTree *Tree, BTNode *Node)
{
    ...
    if (RetSubTreeLeft(Node) != NULL) {
        Insert(Tree, MakeAddressMapWithSubTree(Tree, Node->Left), Node->Data);
    }
    if (RetSubTreeRight(Node) != NULL) {
        Insert(Tree, MakeAddressMapWithSubTree(Tree, Node->Right), Node->Data);
    }
...
{
    DATATYPE Target = *Data;
    Target = Search(Tree, &Target);
    if (Target > 0) {
    	return Find(Tree, &Target);
    } else return Tree->Curr->Data;
}

int Union(BTTree *Tree1, BTTree *Tree2)
{
    int Tree1Height = Search(Tree1, &Tree1->Root->Data);
    int Tree2Height = Search(Tree2, &Tree2->Root->Data);

    DATATYPE Target = 0;

    if (Tree1Height < Tree2Height) {
        if (PosHead(Tree2, &Target)) {
            do {
                InsertBST(Tree1, Target);
            } while(PosNext(Tree2, &Target));
        }
        MakeAddressMap(Tree1);
        return 1;
    } else {
        if (PosHead(Tree1, &Target)) {
            do {
                InsertBST(Tree2, Target);
            } while(PosNext(Tree1, &Target));
        }
